...
Usage:
    python3.9 task.py create "<title>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3] [--parent <dir>] [--package <pkg>] [--no-start]
    python3.9 task.py add-context <dir> <file> <path> [reason] # Add jsonl entry
    python3.9 task.py validate <dir>              # Validate jsonl files
    python3.9 task.py list-context <dir>          # List jsonl entries
    python3.9 task.py start <dir>                 # Set active task
    python3.9 task.py current [--source]          # Show active task
    python3.9 task.py finish                      # Clear active task
    python3.9 task.py set-branch <dir> <branch>   # Set git branch
    python3.9 task.py set-base-branch <dir> <branch>  # Set PR target branch
    python3.9 task.py set-scope <dir> <scope>     # Set scope for PR title
    python3.9 task.py archive <task-dir>          # Archive completed task
    python3.9 task.py list                        # List active tasks
    python3.9 task.py list-archive [month]        # List archived tasks
    python3.9 task.py add-subtask <parent-dir> <child-dir>     # Link child to parent
    python3.9 task.py remove-subtask <parent-dir> <child-dir>  # Unlink child from parent
"""
...
Usage:
  python3.9 task.py create <title>                     Create new task directory
  python3.9 task.py create <title> --package <pkg>     Create task for a specific package
  python3.9 task.py create <title> --parent <dir>      Create task as child of parent
  python3.9 task.py create <title> --no-start          Create without making it active in this session
  python3.9 task.py add-context <dir> <jsonl> <path> [reason]  Add entry to jsonl
  python3.9 task.py validate <dir>                     Validate jsonl files
  python3.9 task.py list-context <dir>                 List jsonl entries
  python3.9 task.py start <dir>                        Set active task
  python3.9 task.py current [--source]                 Show active task
  ...
  python3.9 task.py set-scope <dir> <scope>            Set scope for PR title
  python3.9 task.py archive <task-dir>                 Archive completed task
  python3.9 task.py add-subtask <parent> <child>       Link child task to parent
  python3.9 task.py remove-subtask <parent> <child>    Unlink child from parent
  python3.9 task.py list [--mine] [--status <status>]  List tasks
  python3.9 task.py list-archive [YYYY-MM]             List archived tasks
...
Examples:
  python3.9 task.py create "Add login feature" --slug add-login
  python3.9 task.py create "Add login feature" --slug add-login --package cli
  python3.9 task.py create "Child task" --slug child --parent .trellis/tasks/01-21-parent
  python3.9 task.py add-context <dir> implement .trellis/spec/cli/backend/auth.md "Auth guidelines"
  python3.9 task.py set-branch <dir> task/add-login
  python3.9 task.py start .trellis/tasks/01-21-add-login
  python3.9 task.py current --source
  python3.9 task.py finish
  python3.9 task.py archive add-login
  python3.9 task.py add-subtask parent-task child-task  # Link existing tasks
  python3.9 task.py remove-subtask parent-task child-task
  python3.9 task.py list                               # List all active tasks
  python3.9 task.py list --mine                        # List my tasks only
  python3.9 task.py list --mine --status in_progress   # List my in-progress tasks
""")
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print(
            "  python3.9 ./.trellis/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
