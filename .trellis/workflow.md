...
```bash
python3.9 ./.trellis/scripts/init_developer.py <your-name>
```
...
```bash
python3.9 ./.trellis/scripts/get_context.py --mode packages   # list packages / layers
```
...
```bash
# Task lifecycle
python3.9 ./.trellis/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
python3.9 ./.trellis/scripts/task.py start <name>          # set active task (session-scoped when available)
python3.9 ./.trellis/scripts/task.py current --source      # show active task and source
python3.9 ./.trellis/scripts/task.py finish                # clear active task (triggers after_finish hooks)
python3.9 ./.trellis/scripts/task.py archive <name>        # move to archive/{year-month}/
python3.9 ./.trellis/scripts/task.py list [--mine] [--status <s>]
python3.9 ./.trellis/scripts/task.py list-archive

# Code-spec context (injected into implement/check agents via JSONL).
# `implement.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
# platforms; the AI curates real spec + research entries during planning when needed.
python3.9 ./.trellis/scripts/task.py add-context <name> <action> <file> <reason>
python3.9 ./.trellis/scripts/task.py list-context <name> [action]
python3.9 ./.trellis/scripts/task.py validate <name>

# Task metadata
python3.9 ./.trellis/scripts/task.py set-branch <name> <branch>
python3.9 ./.trellis/scripts/task.py set-base-branch <name> <branch>    # PR target
python3.9 ./.trellis/scripts/task.py set-scope <name> <scope>

# Hierarchy (parent/child)
python3.9 ./.trellis/scripts/task.py add-subtask <parent> <child>
python3.9 ./.trellis/scripts/task.py remove-subtask <parent> <child>

# PR creation
python3.9 ./.trellis/scripts/task.py create-pr [name] [--dry-run]
```

> Run `python3.9 ./.trellis/scripts/task.py --help` to see the authoritative, up-to-date list.
...
```bash
python3.9 ./.trellis/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
python3.9 ./.trellis/scripts/get_context.py                            # full session runtime
python3.9 ./.trellis/scripts/get_context.py --mode packages            # available packages + spec layers
python3.9 ./.trellis/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
```bash
python3.9 ./.trellis/scripts/get_context.py --mode phase --step <step>
# e.g. python3.9 ./.trellis/scripts/get_context.py --mode phase --step 1.1
```
...
```bash
python3.9 ./.trellis/scripts/task.py create "<task title>" --slug <name>
```
...
Skip when `python3.9 ./.trellis/scripts/task.py current --source` already points to a task.
...
```bash
python3.9 ./.trellis/scripts/get_context.py --mode packages
```
...
```bash
python3.9 ./.trellis/scripts/task.py add-context "$TASK_DIR" implement "<path>" "<reason>"
python3.9 ./.trellis/scripts/task.py add-context "$TASK_DIR" check "<path>" "<reason>"
```
...
```bash
python3.9 ./.trellis/scripts/task.py start <task-dir>
```
...
**Final pass (before Phase 3.4 commit)**: the last 2.2 of a task must run full-scope, not just on the latest implement chunk. List all affected packages with `python3.9 ./.trellis/scripts/get_context.py --mode packages`, then load each package's spec index Quality Check section. This catches cross-layer / multi-package issues a mid-iteration local 2.2 cannot.
